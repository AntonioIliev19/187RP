		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Sair para abastecer seu veículo")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Pressione ~g~H ~w~para cancelar o abastecimento do veículo. $~r~" .. price .. " ~w~+  taxa")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Veículo está muito cheio de gás para ser abastecido")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Você atualmente não tem dinheiro suficiente para comprar combustível")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Pressione ~g~H ~w~para abastecer seu veículo. $~r~0.5/~w~galão + taxa")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~H ~w~para cancelar o abastecimento do veículo. Atualmente em: " .. fuel .. "% - Jerry Can: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~H ~w~para abastecer o veículo com sua lata de gasolina")
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetSeatPedIsIn(GetPlayerPed(-1)) == -1 then
		local vehicle = GetPlayersLastVehicle()
		fuel    = math.ceil(round(GetVehicleFuelLevel(vehicle), 1))
		...
		--DrawAdvancedText(0.2195 - x, 0.775 - y, -0.055, 0.0028, 0.6, fuel, 185, 185, 185, 255, 6, 1)

		--DrawAdvancedText(0.130 - x, 0.77 - y, 0.005, 0.0028, 0.6, mph, 255, 255, 255, 255, 6, 1)
		--DrawAdvancedText(0.174 - x, 0.77 - y, 0.005, 0.0028, 0.6, kmh, 255, 255, 255, 255, 6, 1)

		--DrawAdvancedText(0.148 - x, 0.7788 - y, -0.020, 0.0028, 0.5, " Tanque:", 240, 200, 80, 255, 6, 1)
	end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if vehicle and engine then
			...
			if rpm > 0.9 then
				rpmfuelusage = fuel - rpm / 3.1
				Citizen.Wait(2000)
			elseif rpm > 0.8 then
				rpmfuelusage = fuel - rpm / 3.5
				Citizen.Wait(3000)
			elseif rpm > 0.7 then
				rpmfuelusage = fuel - rpm / 3.9
				Citizen.Wait(4000)
			elseif rpm > 0.6 then
				rpmfuelusage = fuel - rpm / 4.4
				Citizen.Wait(5000)
			elseif rpm > 0.5 then
				rpmfuelusage = fuel - rpm / 4.8
				Citizen.Wait(8000)
			elseif rpm > 0.4 then
				rpmfuelusage = fuel - rpm / 5.2
				Citizen.Wait(10000)
			elseif rpm > 0.3 then
				rpmfuelusage = fuel - rpm / 5.4
				Citizen.Wait(12000)
			elseif rpm > 0.2 then
				rpmfuelusage = fuel - rpm / 5.6
				Citizen.Wait(16000)
			else
				rpmfuelusage = fuel - rpm / 5.8
				Citizen.Wait(21000)
			end
