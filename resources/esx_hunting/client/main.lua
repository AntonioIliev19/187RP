...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] Започни лова', ['x'] = -769.23773193359, ['y'] = 5595.6215820313, ['z'] = 33.48571395874 },
	['Sell'] = { ['hint'] = '[E] Продай', ['x'] = 969.16375732422, ['y'] = -2107.9033203125, ['z'] = 31.475671768188 },
...
function StartHuntingSession()
	...
	if OnGoingHuntSession then
		...
		HuntCar = CreateVehicle(GetHashKey('can'), Positions['SpawnATV'].x, Positions['SpawnATV'].y, Positions['SpawnATV'].z, 169.79, true, false)
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Да заколиш животното.', 0.4)
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										ESX.ShowNotification('Трябва да използваш нож!')
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('Ти заколи животното и получи ' ..AnimalWeight.. 'кг месо.')
