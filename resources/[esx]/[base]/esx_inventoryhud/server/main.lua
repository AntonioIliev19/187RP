            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Ти закупи '..count.." "..list[i].label })
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Ти нямаш достатъчно пари!' })
					end
				end
            end
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Ти нямаш достатъчно място в раницата/джобовете!' })
        end
	end
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Ти закупи '..list[i].label })
						else
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Ти нямаш достатъчно пари!' })
						end
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Ти вече имаш това оръжие!' })
					end
				end
            end
        else
		...
		for i = 1, #list, 1 do
			if list[i].name == item then
				...
				if targetWeapon then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Ти не притежаваш това оръжие!' })
				end
            end
        end
    end
