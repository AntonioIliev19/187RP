	...
	while true do
		...
		if CurrentBed ~= nil then
			if not OnBed then
				...
				Draw3DText({x = BedCoords.x, y = BedCoords.y, z = (BedCoords.z+1)}, 'Натисни ~g~[E] ~w~за да легнеш', 0.35)
				
			elseif OnBed then
				local BedCoords = GetEntityCoords(CurrentBed)
				
				Draw3DText({x = BedCoords.x, y = BedCoords.y, z = (BedCoords.z+1)}, 'Натисни ~g~[X] ~w~за да станеш', 0.35)
			end
		end
	end
end)
