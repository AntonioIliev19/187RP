...
local teclatele = 311 --Tecla/key: K, when the phone dont have batery it block that key

local recarregar = {
	{x = -42.74, y = -1754.99, z = 28.45}, -- recharge place
}
...
Citizen.CreateThread(function ()
	while true do
		...
		for _,v in pairs(recarregar) do
			...
			if distance <= 5 then
				Drawing.draw3DText(v.x, v.y, v.z, "[~g~Zona de Carregamento~w~] ~n~Fica dentro do ~b~circulo~w~ ~n~para carregar o ~y~telemovel~w~", 4, 0.15, 0.05, 255, 255, 255, 255)
				DrawMarker(27, v.x, v.y, v.z + 0.2, 0, 0, 0, 0, 0, 0, 0.5,0.5,0.5, 0, 232, 255, 155, 0, 0, 0, 0, 0, 0, 0)
			end
			if bateria < 99 and distance <= 1.3 then
				ESX.ShowNotification("[~g~Bateria do Telemovel~w~]~n~Telemovel a carregar. Bateria: ~g~"..bateria.."%~w~")
				TriggerServerEvent("kuana:updatebateriat")
				Citizen.Wait(2000)
		...
		if (IsControlJustReleased(1, teclatele)) then
			...
			ESX.ShowNotification("[~g~Bateria do Telemovel~w~]~n~O teu telemovel tem "..colorbat..""..bateria.."%~w~~n~ de bateria")
		end
	end
end)
