...
AddEventHandler('trunk',function() 
	local player = GetPlayerPed(-1)
			...
			if distanceToVeh <= interactionDistance then
				...
				ShowNotification("~r~Трябва да си близо до МПС за да извършиш тази дейност.")
			end
end)
-- R E A R  D O O R S --
	...
	local distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(vehicle), 1)
		if DoesEntityExist(vehicle) then
			if distanceToVeh <= lockDistance then
				...
				ShowNotification("Ти ~r~заключи ~w~твоето ~y~МПС~w~.")
				TriggerEvent('lockLights')
				else
				SetVehicleDoorsLocked(vehicle,1)
				ShowNotification("Ти ~g~отключи ~w~твоето ~y~МПС~w~.")
				TriggerEvent('lockLights')
				end
			else
				ShowNotification("~r~Трябва да си близо до МПС за да извършиш тази дейност.")
			...
			ShowNotification("~r~Нямаш ключове за това МПС.")
		end
	end)
...
AddEventHandler('save',function() 
	...
	if (IsPedSittingInAnyVehicle(player)) then 
		if  saved == true then
			...
			ShowNotification("Ти ~r~остави ~w~ключовете.")
			saved = false
		else
			RemoveBlip(targetBlip)
			...
			ShowNotification("Ти ~g~взе ~w~ключовете за това ~y~МПС~w~.")
			saved = true
		end
	end
end)
...
AddEventHandler('controlsave',function() 
		if controlsave_bool == false then
			...
			ShowNotification("~r~Ти нямаш ключове за това МПС.")
			else
			ShowNotification("Ти нямаш повече контрол над твоето ~y~МПС~w~.")
			end
		else
			controlsave_bool = false
			if saveVehicle == nil then
