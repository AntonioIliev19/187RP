...
function ShowNoPhoneWarning () 
  ...
  ESX.ShowNotification("Вие нямате ~r~телефон~s~")
end
...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ...
      AddTextComponentString("~g~" .. data.name .. ' ~o~' .. number .. '~n~~INPUT_PICKUP~~w~ Използвай')
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    local text = '~o~Ново съобщение'
    if ShowNumberNotification == true then
      text = '~o~Ново съобщение от ~y~'.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = '~o~Ново съобщение от ~g~'.. contact.display
          break
        end
      end
    end
