...
function OpenSimMenu()
	...
	local elements2 = {}
	  ...
	  function(data, menu)
		...
		local elements2 = {
			{label = 'Използвай', value = 'sim_use'},
			{label = 'Дай', value = 'sim_give'},
		  	{label = 'Счупи', value = 'sim_delete'}
		  }
  
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sim_change', {
			title    = "Какво искате да направите с тази сим карта?",
			align    = 'center',
			elements = elements2,
		  ...
		  function(data2, menu2)
			...
			if data2.current.value == 'sim_use' then
				...
				ESX.ShowNotification("Активирали сте сим картата ~o~" .. data.current.value.number)
			...
			if data2.current.value == 'sim_give' then
				...
				if closestPlayer == -1 or closestDistance > 3.0 then
					ESX.ShowNotification('Няма човек наблизо')
				else
			...
			if data2.current.value == 'sim_delete' then
				...
				ESX.ShowNotification("Изхвърлихте сим картата ~o~" .. data.current.value.number)
				TriggerServerEvent('gcPhone:allUpdate')
			end
		...
		if data.current.value == 'open_phone' then
			...
			ESX.TriggerServerCallback('esx_cartesim:CheckSim', function(sim)
				if sim then
					...
					ESX.ShowNotification("Нямате сим карта !")
				end
			end)
