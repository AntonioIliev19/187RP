		...
		MySQL.Async.fetchAll(
			...
			function(result)
				...
				MySQL.Async.execute(
					...
					function (rowsChanged)
					TriggerClientEvent('esx:showNotification', source, "Имате нова сим карта")
					end
				)
		end)
end
...
AddEventHandler('esx_cartesim:sim_give', function (store, index)
  ...
  TriggerClientEvent('esx:showNotification', _source, "Дадохте сим картата ~o~" .. simZ)
  TriggerClientEvent('esx:showNotification', id, "Получихте сим карта ~o~" .. simZ)
