...
AddEventHandler('route68_ruletka:start', function()
	ESX.TriggerServerCallback('esx_roulette:check_money', function(quantity)
		if quantity >= 10 then
			...
			ESX.ShowNotification('Трябва да заложите поне 10 жетона!')
			SendNUIMessage({
				type = "reset_bet"
			})
		end
...
AddEventHandler('esx_roulette:start_game', function(action, amount)
	...
	if game_during == false then
		...
		TriggerEvent('pNotify:SendNotification', {text = "Заложихте "..amount.." жетона на "..kolorBetu.."."})
		game_during = true
		...
		if action == 'black' then
			if has_value(black, randomNumber) then
				local win = amount * 2
				ESX.ShowNotification('Спечелихте '..win..' жетона!')
				TriggerServerEvent('esx_roulette:givemoney', action, amount)
			else
				ESX.ShowNotification('Изгубихте. Опитайте отново. КЪСМЕТ!')
			end
		elseif action == 'red' then
			local win = amount * 2
			if has_value(red, randomNumber) then
				ESX.ShowNotification('Ти спечели '..win..' жетона!')
				TriggerServerEvent('esx_roulette:givemoney', action, amount)
			else
				ESX.ShowNotification('Изгубихте. Опитайте отново. КЪСМЕТ!')
			end
		...
		ESX.ShowNotification('Завъртане..')
	end
end)