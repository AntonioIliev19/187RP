...
Citizen.CreateThread(function()
	while true do
		...
		local Dystans = GetDistanceBetweenCoords(PozycjaGracza, 977.007, 39.410, 74.882, true)
		local Dystans2 = GetDistanceBetweenCoords(PozycjaGracza, 968.107, 34.904, 74.876, true)
		...
		if Dystans <= 10.0 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu, "Натиснете [~g~E~s~] за да купите жетони", 0.55, 1.5, "~b~КАСИЕР", 0.7)
		...
		if Dystans2 <= 6.0 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu2, "Натиснете [~g~E~s~] за да отворите барa", 0.55, 1.5, "~b~БАР", 0.7)
...
function OtworzMenuKasyna()
	ESX.UI.Menu.Open(
      ...
      {
          title    = 'Diamond Casino - CAISSE',
          align    = 'left',
          elements = {
			{label = "Купете жетони", value = "buy"},
			{label = "Разменете жетони", value = "sell"},
		  }
      },
	  function(data, menu)
		...
		elseif akcja == 'sell' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
				...
				if quantity == nil then
					TriggerEvent("pNotify:SendNotification", {text = 'Грешна сума.'})
				else
...
function OtworzMenuKasynaSklepu()
	ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'alkohole',
      {
          ...
          align    = 'left',
          elements = {
			{label = "Бира", value = "beer"},
			{label = "Уиски", value = "whisky"},
			{label = "Текила", value = "tequila"},
...
RegisterNUICallback('wygrana', function(data)
	TriggerEvent('pNotify:SendNotification', {text = 'Вие печелите '..data.win..' жетони!'})
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #Config.Sloty do
			...
			if dis <= 2.0 then
				ESX.ShowHelpNotification('Натиснете ~INPUT_PICKUP~ за да играете на машините.')
		...
		for i=1, #Config.Ruletka do
			...
			if dis <= 2.0 then
				ESX.ShowHelpNotification('Натиснете ~INPUT_PICKUP~ за да играете на рулетката.')
		...
		for i=1, #Config.Blackjack do
			...
			if dis <= 2.0 then
				ESX.ShowHelpNotification('Натиснете ~INPUT_PICKUP~ да играете BlackJack.')
