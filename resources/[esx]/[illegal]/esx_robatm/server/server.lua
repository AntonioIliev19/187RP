...
AddEventHandler('esx_robatm:toofar', function(robb)
	local source = source
	...
	if(robbers[source])then
		TriggerClientEvent('esx_robatm:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Banks[robb].nameofbank)
	end
end)

RegisterServerEvent('esx_robatm:rob')
AddEventHandler('esx_robatm:rob', function(robb)
	...
	if Banks[robb] then
		...
		if rob == false then
		  ...
		  if xPlayer.getInventoryItem('lom').count >= 1 then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('esx_robatm:setblip', xPlayers[i], Banks[robb].position)
					end
				end
				...
				TriggerClientEvent('esx_robatm:currentlyrobbing', source, robb)
				...
				SetTimeout(30000, function()
					...
					if(robbers[savedSource])then
						...
						rob = false
						TriggerClientEvent('esx_robatm:robberycomplete', savedSource, job)
						if(xPlayer)then
