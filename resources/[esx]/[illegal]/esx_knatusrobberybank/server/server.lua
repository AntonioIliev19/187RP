...
RegisterServerEvent('esx_holdupbank:printFrozenDoors')
AddEventHandler('esx_holdupbank:printFrozenDoors', function()
...
RegisterServerEvent('esx_holdupbank:setDoorFreezeStatus')
AddEventHandler('esx_holdupbank:setDoorFreezeStatus', function(bank, door, status)
    local src = source
    local xPlayers = ESX.GetPlayers()
    local cops = 0
    for i = 1, #xPlayers do
    ...
    if cops >= Config.BankRobbery[bank].Cops or xPlayer.job.name == 'police' then
        ...
        TriggerClientEvent('esx_holdupbank:setDoorFreezeStatusCl', -1, bank, door, status)
    else
        TriggerClientEvent('esx:showNotification', src, 'Det är inte tillräckligt med poliser online!') -- prob lua executor (or bug)
    end
end)

RegisterServerEvent('esx_holdupbank:getDoorFreezeStatus')
AddEventHandler('esx_holdupbank:getDoorFreezeStatus', function(bank, door)
    TriggerClientEvent('esx_holdupbank:setDoorFreezeStatusCl', source, bank, door, Config.BankRobbery[bank].Doors[door].Frozen)
end)
