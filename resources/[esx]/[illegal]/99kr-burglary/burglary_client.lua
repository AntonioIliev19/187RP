...
local chancePoliceNoti        = 75         -- the procent police get notified (only numbers like 30, 10, 40. You get it.)
...
local text = "Натиснете ~g~[E]~w~ за да използвате отключалката" -- lockpick the door
local textUnlock = "Натиснете ~g~[E]~w~ за да влезете" -- enter the house
local insideText = "Натиснете ~g~[E]~w~ за да излезнете" -- exit the door
local searchText = "~g~[E]~w~ претърси" -- search the spot
local emptyMessage = "Тук нямаше нищо!" -- if you press E where it is empty
local emptyMessage3D = "~r~Празно" -- if the spot is empty
local closetText = "~g~[E]~w~ Погледнете в килера" -- text at closet

local lockpickQuestionText = "Искате ли да разбиете вратата?" -- confirmation menu title
local noLockpickText = "Вие нямате отключалки!" -- if you don't have a lockpick and you try to do the burglary
local yesText = "Да" -- in confirmation menu
local noText = "Не" -- -- in confirmation menu

local shirt = "Блузи" -- in menu and notification
local pants = "Панталони" -- -- in menu and notification
local hat = "Шапки" -- -- in menu and notification
local shoes = "Обувки" -- -- in menu and notification

local youFound = "Вие намерихте" -- when you steal something
local burglaryDetected = "Открито е влизане в взлом" -- text 1 cops gets sent
local sentPhoto = "Изпратихме ви снимка от камерите." -- if you use qalle's camerasystem this will be in the message too
...
local burglaryInside = {
    ["Поправителен комплект"] = { x = 342.23, y = -1003.29, z = -99.0, item = 'repairkit', amount = math.random(2,4)},
    ["Шперцове"]  = {x = 338.14, y = -997.69, z = -99.2, item = 'lockpick', amount = math.random(1,2)},
    ["ОС за хакване"] = { x = 350.91 , y = -999.26 , z = -99.2, item = 'raspberry', amount = math.random(1,2)},
    ["Пликче кока"] = { x = 349.19, y = -994.83, z = -99.2, item = 'coke_pooch', amount = math.random(1,2)},
    ["Горелка"]  = { x = 345.3, y = -995.76, z = -99.2, item = 'blowtorch', amount = math.random(1,2)},
    ["Амфети"] = { x = 346.14, y = -1001.55, z = -99.2, item = 'Амфети', amount = math.random(1,3)},
    ["Билет за пералнята на пари"] = { x = 347.23, y = -994.09, z = -99.2, item = 'moneywashid', amount = math.random(1,2)},
    ["Бомба"] = { x = 339.23, y = -1003.35, z = -99.2, item = 'c4_bank', amount = math.random(1,5)}
}
            ...
            if useQalleCameraSystem == true then
              if random <= chancePoliceNoti then                           -- chance police get notified and they get a photo of you
                TriggerEvent('skinchanger:getSkin', function(skin)
                  ...
                  TriggerServerEvent('gcphone:send', 'police', burglaryDetected .. ' ' .. house .. '. ' .. sentPhoto, { x = v.pos.x, y = v.pos.y, z = v.pos.z })
                end)
              end
            end
              if useQalleCameraSystem == false then
                if random <= chancePoliceNoti then
                  TriggerServerEvent('gcphone:send', 'police', burglaryDetected .. ' ' .. house, { x = v.pos.x, y = v.pos.y, z = v.pos.z })
               end
            end
        else 
