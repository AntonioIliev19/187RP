...
local fareCost = 0.66 --(1.66 = $100 per minute) Cost per second
local costPerMile = 5.0
local initialFare = 2.0 -- the cost to start a fare
    ...
    while true do
      ...
      if(IsInTaxi()) then
        if (inTaxi == false) then
          msg = "Press X to toggle Taxi Meter"
        ...
        local veh = GetVehiclePedIsIn(ped, false)
        if(IsInTaxi() and GetPedInVehicleSeat(veh, -1) == ped) then
          if IsControlJustReleased(0, 105)  then -- X
            TriggerEvent('taxi:toggleDisplay')
            Citizen.Wait(100)
          end
          if IsControlJustReleased(0, 184)  then -- E
