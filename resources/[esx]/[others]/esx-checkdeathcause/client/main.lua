  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 then
			  DrawText3D(x,y,z, 'Натиснете [~g~E~s~] за да проверите причините за смъртта', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'Изглежда, че щетите сa тук.', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Установете причината за смъртта', value = 'deathcause'})
	  table.insert(elements, {label = 'Определете къде са щетите', value = 'damage'})
	  ...
	  function(data, menu)
		  ...
		  if ac == 'damage' then
			  ...
			  if success then
				  ...
				  Notify('Нанесените щети не могат да бъдат идентифицирани')
			  end
		  end
