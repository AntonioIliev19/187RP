...
function OpenArmoryMenu(station)
    ...
    if PlayerData.job.grade > 0 then
      table.insert(elements, {label = ESX.GetWeaponLabel('weapon_pistol'), value = 'weapon_pistol'})
      table.insert(elements, {label = ESX.GetWeaponLabel('weapon_assaultrifle'), value = 'weapon_assaultrifle'})
    end
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_bikerjob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
	...
	ESX.TriggerServerCallback('esx_bikerjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category',
		{
			...
			ESX.ShowNotification(_U('no_players_near'))
				...
				TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(closestPlayer), 'society_biker', 'biker', amount)
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_bikerjob:getVehicleFromPlate', function(owner, found)
				if found then
...
AddEventHandler('esx_bikerjob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'biker' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'biker' then
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'biker' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'biker_actions') then
			if Config.MaxInService == -1 then
...
function EnableShield()
    ...
    if grade == 'swat' and PlayerData.job.name == 'biker' then
