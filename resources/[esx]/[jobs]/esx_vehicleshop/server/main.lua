...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function(vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		...
		esx:showNotification(_U('vehicle_belongs', vehicleProps.plate))
	end)
end)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
				...
				esx:showNotification(_U('have_withdrawn', count, item.label))
			else
				esx:showNotification(_U('player_cannot_hold'))
			end
		else
			esx:showNotification(_U('not_enough_in_society'))
		end
	end)
end)
...
AddEventHandler('esx_vehicleshop:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if item.count >= 0 then
			...
			esx:showNotification(_U('have_deposited', count, item.label))
		else
...
AddEventHandler('esx_vehicleshop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			esx:showNotification(_U('vehicle_sold_for', vehicleModel, ESX.Math.GroupDigits(price)))
		else
	...
	MySQL.Async.fetchAll('SELECT * FROM rented_vehicles', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then -- message player if connected
				...
				esx:showNotification(_U('paid_rental', ESX.Math.GroupDigits(result[i].rent_price)))
