...
local price = 0
if Config.Zones[CurrentActionData.zone].legal == 1 then
	price = 500
else
	price = 200
end

table.insert(elements, {label = '---  Муниции  ---', value = nil})

table.insert(elements, {
    label     = 'Патрони' .. ' - <span style="color:green;">€' .. price .. ' </span>',
    value     = 'clip'
}
  ...
  ESX.UI.Menu.Open(
	...
	TriggerServerEvent('esx_weashop:buyCaricatore',	zone)
	else
	TriggerServerEvent('esx_weashop:buyCaricatore',	zone)
	end
	end
