    ...
    while true do
        ...
        for k,v in pairs(Config.Zones) do
            if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance and k == 'Pimp' then
                letSleep = false
                DrawText3Ds(v.Pos.x, v.Pos.y, v.Pos.z+1.0, "Натисни ~b~[E]~w~ за да си поръчаш проститутка")
...
RegisterNUICallback("ChooseCathrine", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Cathrine е отбелязана на GPS, отиди да я вземеш.")
...
RegisterNUICallback("ChooseTatiana", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Tatiana е отбелязана на GPS, отиди да я вземеш.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseBootylicious", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Bootylicious е отбелязана на GPS, отиди да я вземеш.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseVennesa", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Vennesa е отбелязана на GPS, отиди да я вземеш.")
    OnRouteToHooker = true
end)
...
AddEventHandler("sawu_hookers:ChosenHooker", function(model)
    if HookerSpawned then
        exports['mythic_notify']:SendAlert('error', "Ти вече си си поръчал проститутка!")
    else
        HookerSpawned = true
        CreateHooker(model)
        Citizen.CreateThread(function()
                ...
                if HookerInCar then
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and not IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                        ...
                        if IsVehicleStopped(vehicle) then
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, '[~b~E~w~] За да видиш какво предлага [~r~H~w~] Кажи на курвата да си ходи')
                            if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, 'Закарай я на закрито')
                        end
                    end
                end
                if letSleep then
