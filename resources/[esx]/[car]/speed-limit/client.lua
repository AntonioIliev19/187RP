	...
	while true do 
		...
		local float Max = GetVehicleMaxSpeed(vehicleModel)
			if ( ped ) then
				...
				SendNUIMessage({
						cruise = true,
						TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Ограничение на скоростта - включено!' })
					})else
					SetEntityMaxSpeed(vehicle, Max)
					enableCruise = false
					TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Ограничение на скоростта - изключено!' })
					SendNUIMessage({
						cruise = false,
					})
					end 
						else
							TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Не си в превозно средство!' })
						end
					end
				end
			end
