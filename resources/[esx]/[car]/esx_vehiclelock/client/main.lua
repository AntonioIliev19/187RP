		...
		for i=1, #mykey, 1 do
			if mykey[i].got == 'true' then
				if 	mykey[i].NB == 1 then
						table.insert(elements, {label = 'Ключове : '.. ' [' .. mykey[i].plate .. ']', value = mykey[i].plate})
					elseif mykey[i].NB == 2 then
						table.insert(elements, {label = '[DOUBLE] МПС : '.. ' [' .. mykey[i].plate .. ']', value = nil})
					end
				end
			end
	...
	'default', GetCurrentResourceName(), 'mykey',
	{
        ...
        ESX.UI.Menu.CloseAll()
  			ESX.UI.Menu.Open(
				'default', GetCurrentResourceName(), 'mykey',
				{
				title = 'Бихте ли искали ?',
				align = 'right',
				elements = {
						{label = 'Смяна на собственика', value = 'donnerkey'}, -- Donné les clés
						{label = 'Заеми ключовете си', value = 'preterkey'}, -- Donné les clés
			  		},
	  			},
	...
	'default', GetCurrentResourceName(), 'mykey',
	{
        ...
        ESX.UI.Menu.CloseAll()
  			ESX.UI.Menu.Open(
				...
				align = 'right',
				elements = {
						{label = 'прет', value = data2.current.value}, -- Donné les clés
			  		},
	  			},
        		function(data3, menu3) --Submit Cb
 					local player, distance = ESX.Game.GetClosestPlayer()
