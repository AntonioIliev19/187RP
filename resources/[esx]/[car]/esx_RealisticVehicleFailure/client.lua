	...
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			if GetIsVehicleEngineRunning(vehicle) then
				notification("~g~Двигателя трябва да е изключен по време на поправка.")
				return
			else
				...
				if PlayerData.job.name == 'mechanic' then
					...
					notification("~g~Ти поправи колата!")
					return			
				else
					if mechNumb == 1 then
						notification("~g~Механика ти преглежда колата.")
						Citizen.Wait(11000)
						notification("~g~Механика работи по колата ти.")
						Citizen.Wait(11000)
						if GetIsVehicleEngineRunning(vehicle) then
							notification("~g~Двигателя трябва да е изключен по време на поправка.")
							return
