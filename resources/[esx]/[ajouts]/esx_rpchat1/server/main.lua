...
RegisterCommand('news', function(source, args, rawCommand)
  ...
  local name = getIdentity(source)
  if xPlayer.job.name == 'mechanic' or xPlayer.job.name == 'cardealer' or xPlayer.job.name == 'ammu' or xPlayer.job.name == 'journaliste' or xPlayer.job.name == 'unicorn' or xPlayer.job.name == 'nightclub' or xPlayer.job.name == 'bahama' or xPlayer.job.name == 'taxi' then
  fal = name.firstname .. " " .. name.lastname
  TriggerClientEvent('chat:addMessage', -1, {
      template = '<div class="chat-message-ad"><b>Реклама :</b> {1}</div>',
      args = { fal, msg }
  })
  end
end, false)
